...
bool sortKeys(vector<int> A,vector<int>B)
{
    if(A.size()==B.size())
    {
        return A<B;
...
int main()
{
    ...
    for(int i = 0 ; i<Ans.size();i++)
    {
        ...
        {
            if(checkSubset(Ans[i],CandidateKeys[j])==true)
            {
                var= 0;
                break;
            }
        ...
        if(var==1)
        {
            CandidateKeys.push_back(Ans[i]);
        }
    }
    sort(CandidateKeys.begin(),CandidateKeys.end(),sortKeys);
    cout<<CandidateKeys.size()<<endl;
    for(int i = 0 ; i< CandidateKeys.size();i++)
    {
        for(int j = 0 ; j<CandidateKeys[i].size();j++)
        {
            cout<<CandidateKeys[i][j]<<" ";
        }
        cout<<endl;
    }
